...
class Reviews(db.Model):
    ...
    def __init__(self,pub_id):
        ...
        self.user_id="chyba coś jest niedokończone, bo ta recenzja nie jest przypisana do żadnego użytkownika :("
    ...
    def user_id_json(self):
        return {'user_id' : self.pub_id}
    ...
    def json_f(self):
        return {'stars':self.stars, 'opinion': self.opinion, 'pub_id': self.pub_id, 'user_id': self.user_id,  'ID' : self.review_id}

    def __str__(self):
        return "{} and {} and {} and {}".format(self.review_id, self.pub_id, self.user_id, self.opinion, self.stars)